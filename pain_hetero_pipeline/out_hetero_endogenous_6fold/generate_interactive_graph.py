#!/usr/bin/env python3
"""Generate interactive HTML visualization for Pain HeteroGNN network.
   Includes ALL metabolites, ALL enzymes, ALL sub-pathways.
"""
...
# Paths
BASE_6FOLD = "/home/ryu11/PPI_GNN/pain_hetero_pipeline/out_hetero_endogenous_6fold"
...
OUTPUT_HTML = os.path.join(BASE_6FOLD, "interactive_graph.html")

# ---------- Read data ----------

# metabolite_rank.csv
metabolites = []
with open(os.path.join(BASE_6FOLD, "metabolite_rank.csv")) as f:
...
# enzyme_rank.csv
enzymes = []
with open(os.path.join(BASE_6FOLD, "enzyme_rank.csv")) as f:
    reader = csv.DictReader(f)
    for row in reader:
        enzymes.append({
            ...
            "evidence_counts": int(row["evidence_counts"]),
            "selection_frequency": int(row["selection_frequency"]),
        })
enzymes.sort(key=lambda x: x["enzyme_score"], reverse=True)

# cv_metrics_overall.csv
cv_overall = []
with open(os.path.join(BASE_6FOLD, "cv_metrics_overall.csv")) as f:
    reader = csv.DictReader(f)
    for row in reader:
        cv_overall.append(row)

# cv_metrics_by_group.csv
cv_groups = []
with open(os.path.join(BASE_6FOLD, "cv_metrics_by_group.csv")) as f:
    reader = csv.DictReader(f)
    for row in reader:
        cv_groups.append(row)

# met2ec.json
with open(os.path.join(BASE_6FOLD, "met2ec.json")) as f:
    met2ec = json.load(f)

# met2pathway.json
with open(os.path.join(BASE_6FOLD, "met2pathway.json")) as f:
...
# ---------- Use ALL metabolites and ALL enzymes ----------
all_metabolites = metabolites  # ALL 1019
all_enzymes = enzymes          # ALL 966
...
# Filter out path:map01100 from enzyme pathways display strings
for e in all_enzymes:
    pws = [p.strip() for p in e["pathways"].split(";") if "map01100" not in p]
    e["pathways"] = "; ".join(pws)

# Annotate enzymes with ec_info
for e in all_enzymes:
    info = ec_info.get(e["enzyme_id"], {})
    e["name"] = info.get("name", e["enzyme_id"])
...
# Metabolite nodes - ALL metabolites with tiered sizing
for i, m in enumerate(all_metabolites):
    ...
    # Tiered sizing and opacity
    if rank <= 50:
        # Top 50: larger (20-45px), full color
        min_s = all_metabolites[49]["importance_score"] if len(all_metabolites) > 49 else 0
        ...
        # Rank 51-200: medium (14-20px), full color
        min_s = all_metabolites[min(199, len(all_metabolites)-1)]["importance_score"]
        ...
        # Rank 201+: small (8-12px), lighter fill (60% opacity)
        min_s = all_metabolites[-1]["importance_score"]
        max_s = all_metabolites[200]["importance_score"] if len(all_metabolites) > 200 else all_metabolites[-1]["importance_score"]
        norm = (score - min_s) / (max_s - min_s) if max_s > min_s else 0.5
...
enz_scores = [e["enzyme_score"] for e in all_enzymes]
for i, e in enumerate(all_enzymes):
    ...
    if rank <= 30:
        ...
        min_s = all_enzymes[min(29, len(all_enzymes)-1)]["enzyme_score"]
        ...
        min_s = all_enzymes[min(99, len(all_enzymes)-1)]["enzyme_score"]
        ...
        max_s = all_enzymes[min(100, len(all_enzymes)-1)]["enzyme_score"]
        norm = (score - min_s) / (max_s - min_s) if max_s > min_s else 0.5
...
<title>Pain HeteroGNN Network Explorer</title>
...
.group-metrics {{ display:flex; gap:8px; }}
.group-box {{ flex:1; background:#0f172a; border-radius:8px; padding:8px 12px; border:1px solid #334155; }}
.group-box .gname {{ font-size:11px; color:#94a3b8; }}
.group-box .gval {{ font-size:16px; font-weight:700; color:#38bdf8; }}
...
.detail-row .dval {{ font-size:13px; color:#e2e8f0; margin-top:2px; }}
...
<div id="sidebar">
  <div id="sidebar-header">
    <h1>Pain HeteroGNN Network Explorer</h1>
    <p>Interactive metabolite-enzyme-pathway network from 6-fold CV (all nodes)</p>
  </div>
...
""" + "".join(f'      <div class="group-box"><div class="gname">{g["group"]} (n={g["n_samples"]})</div><div class="gval">AUC {float(g["auc"]):.3f}</div></div>\n' for g in cv_groups) + """
    </div>
  </div>
...
function showDetail(node) {
  ...
  if (d.type === 'metabolite') {
    ...
    html += detailRow('Importance Score', d.importance_score ? d.importance_score.toExponential(3) : 'N/A');
    html += detailRow('Score Std', d.importance_std ? d.importance_std.toExponential(3) : 'N/A');
    ...
    html += detailRow('Enzyme Score', d.enzyme_score.toExponential(3));
    html += detailRow('Score Std', d.score_std.toExponential(3));
    html += detailRow('Selection Frequency', d.selection_frequency + '/6');
    html += detailRow('Evidence Counts', d.evidence_counts);
